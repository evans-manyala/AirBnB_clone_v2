...
class TestDBStorage(unittest.TestCase):
    '''testing databse storage engine'''
    def testNewSave(self):
        ...
        newUser = User(**{'first_name': 'Willy',
                          'last_name': 'Wonka',
                          'email': 'willy@Wonka.com',
                          'password': 456321})
        cur = db.cursor()
        cur.execute('SELECT COUNT(*) FROM users')
        oldCount = cur.fetchall()
        cur.close()
        db.close()
        newUser.save()
        db = MySQLdb.connect(user=os.getenv('HBNB_MYSQL_USER'),
        ...
        cur = db.cursor()
        cur.execute('SELECT COUNT(*) FROM users')
        newCount = cur.fetchall()
        self.assertEqual(newCount[0][0], oldCount[0][0] + 1)
        cur.close()
        db.close()

    def testNew(self):
        """ New object is successfully added to database """
        new = User(
        ...
        self.assertIn('jane.doe@ymail.com', result)
